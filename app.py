    ...
    def check_all_books(self):
        ...
        for name in self.player_order:
            ...
            if not self.players[name].get("hand") and self.deck:
                ...
                print(f"DEBUG: {name} was empty, drew 1 card. Hand is now {len(self.players[name]['hand'])}")

    def next_turn(self):
        """Cycles to the next player who has cards. Prevents dead ends."""
        ...
        for _ in range(num_players):
            ...
            if self.players[self.player_order[self.turn_index]]["hand"]:
                return # Found a player with cards
        # If loop finishes, no one has cards. get_state will handle gameOver.
...
def ask():
    ...
    if not game.players[asker]["hand"] and not game.deck:
        game.next_turn()
        return jsonify({"message": f"{asker} has no cards and the ocean is empty. Passing...", "state": game.get_state(asker)})
    
    # If they have no cards but the DECK exists, they should have drawn in check_all_books.
    # If for some reason they are still empty, force a draw here.
    ...
    if drawn_rank == rank:
        return jsonify({"message": f"Go Fish! But you caught a {rank}! Go again.", "state": game.get_state(asker), "goFish": False})
...
def ai_move():
    room_id = request.json.get('room_id')
    game = rooms.get(room_id)
    if not game or game.player_order[game.turn_index] != "Computer": return jsonify({})

    hand = game.players["Computer"]["hand"]
    if not hand:
        game.next_turn()
        return jsonify({"message": "Computer passes.", "state": game.get_state("Player")})
    ...
    if matches:
        ...
        game.check_all_books()
        return jsonify({"message": f"🤖 Computer took your {rank_to_ask}s!", "state": game.get_state(target)})
    
    if game.deck: game.players["Computer"]["hand"].append(game.deck.pop())
    game.check_all_books()
    game.next_turn()
    return jsonify({"message": f"🤖 Computer asked for {rank_to_ask}s and fished.", "state": game.get_state(target)})
